...
public class Chessx2Application {
	...
	public String makinggif(@RequestBody Map<String,String> data) throws IOException {
		...
		try {
			...
			//
			String scriptPath = getClass().getClassLoader().getResource("makingGif.py").getPath().substring(1);
			System.out.println("cur here " + scriptPath);
			ProcessBuilder processBuilder = new ProcessBuilder(
					"python3",
					scriptPath,         // Python 스크립트 파일 경로
					"--input_dir", input_dir,  // input_dir 인수
					"--output_dir", output_link + "/alpa.gif", // output_dir 인수
					"--delay", "500"         // delay 인수
			);

//			System.out.println("test here " +  output_link + "/alpa.gif");
